...
int BootLoadFlashCD(int cdromId, int ExpectedFileSize) {
	...
	if (!cdPresent) {
		...
		//VIDEO_CURSOR_POSY += 180;
		//VIDEO_CURSOR_POSX += 80;
		ClearScreen();
		printk("\n\n\n\n\n\n\n\n\n\n");
		printk("                    X-Changer Flasher: Insert disk and press A\n");
...
int BootLoadUpdateFlash(int cdromId) {
	...
	I2CTransmitWord(0x10, 0x0c01); // close DVD tray
	for (n=0;n<16;++n) {
		if((BootIso9660GetFile(cdromId,"/update.bin", (u8 *)KERNEL_PM_CODE, 0x10)) >=0 ) {
			cdPresent=1;
			break;
		}
		wait_ms(250);
	...
	if (!cdPresent) {
		...
		ClearScreen();
		printk("\n\n\n\n\n\n\n\n\n\n");
		printk("                    X-Changer UPDATE: Insert disk and press A\n");
	...
	//printk("\n\nCDROM: ");
	//printk("Loading update from CDROM:/update.bin. \n");
	dwConfigSize=BootIso9660GetFile(cdromId, "/update.bin", (u8 *)KERNEL_PM_CODE, 	256*1024);
	
	if( dwConfigSize < 0 ) { //It's not there
		printk("\n\n                    update.bin not found on CDROM... Halting\n");
		while(1) ;
	}

	printk("\n\n                    File size: %i\n", dwConfigSize);
        if (dwConfigSize!=256*1024) {
		printk("                    File is not 256k - aborted\n");
		while (1);
	}
	SHA1Reset(&context);
	SHA1Input(&context,(u8 *)KERNEL_PM_CODE,dwConfigSize);
