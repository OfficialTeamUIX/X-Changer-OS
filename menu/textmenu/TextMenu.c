...
void TextMenuDraw(TEXTMENU* menu, TEXTMENUITEM *firstVisibleMenuItem, TEXTMENUITEM *selectedItem) {
	...
	VIDEO_CURSOR_POSX=60; //75;
	VIDEO_CURSOR_POSY=175; //125;
	
	//Draw the menu title.
	VIDEO_ATTR=HEADCOLOR;
	printk("\2          %s\n",menu->szCaption);
	VIDEO_CURSOR_POSY+=10;
	...
	for (menucount=0; menucount<8; menucount++) {
		...
		if (item == selectedItem) VIDEO_ATTR=SELCOLOR;
		else VIDEO_ATTR=NONSELCOLOR;
		//Font size 2=big.
		printk("\n\2               %s\n",item->szCaption);
